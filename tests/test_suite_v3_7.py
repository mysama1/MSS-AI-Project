"""Full test suite for v3.7 (v3.4 + post-processing filter v3)"""
import subprocess
import json
import sys
import re

# Post-processing filter - comprehensive v3
FORBIDDEN_MAP = {
...
def test_model(prompt):
    result = subprocess.run(
        ["ollama", "run", "mss-ai-v3_5", prompt],
        capture_output=True, text=True, timeout=60,
        encoding='utf-8', errors='ignore'
    )
    return filter_output(result.stdout)
...
def analyze_response(response, category):
    ...
    if category == "A1":
        ...
        if "0/1" in response or "binary" in response.lower() or "discernment" in response.lower():
            score += 0.3
            details.append("binary_operation")
    elif category == "A3":
        if "self" in response.lower() and ("reference" in response.lower() or "refer" in response.lower()):
            score += 0.3
            details.append("self_reference")
    elif category == "A4":
        if "entropy" in response.lower() and ("increase" in response.lower() or "closed" in response.lower()):
            score += 0.3
            details.append("entropy_increase")
    elif category == "A5":
        if "contradiction" in response.lower() and ("dimension" in response.lower() or "elevate" in response.lower() or "higher" in response.lower()):
            score += 0.3
            details.append("contradiction_elevation")
    elif category == "A6":
