...
class PlanAgent(BaseAgent):
    ...
    def detect_coupling(self, source_agent: str, result: dict[str, Any]) -> list[CouplingSignal]:
        ...
        coupling_rules = {
            ...
            "code": ["Code-Agent"],
            "kb": ["KB-Agent"],
            "api": ["Code-Agent", "Product-Agent"],
            "model": ["Video-Agent", "Code-Agent"],
            "translate": ["Translate-Agent"],
            "publish": ["Product-Agent"],
        }

        # 从结果中提取关键词
        result_text = json.dumps(result, ensure_ascii=False).lower()
        for keyword, targets in coupling_rules.items():
    ...
    def check_pollution(self, agent_name: str, content: str) -> bool:
        ...
        if g_result:
            waste = getattr(g_result, 'waste', 0)
            meaning = getattr(g_result, 'meaning', 1)
            if waste >= 0.7:
                is_polluted = True
                reasons.append(f"Waste score={waste:.2f}")
            if meaning <= 0.2:
                is_polluted = True
                reasons.append(f"Meaning score={meaning:.2f}")
