...
# NOTE: heat_tax_accountant.py was never implemented;
# tool_budget_gate now uses HeatTaxBudget directly.
...
class ToolBudgetGate:
    ...
    def approve(
        ...
        if category in (ToolCategory.PHILOSOPHY, ToolCategory.OVERKILL):
            # Allow only if L2 meaning tax has room (< 0.5 threshold)
            l2_spent = self.accountant.spent.get(HeatTaxLevel.L2_MEANING, 0)
            if l2_spent > 0.5:
                return GateDecision(
                    tool_name=tool_name,
                    category=category,
        ...
        # Record via HeatTaxBudget
        if category == ToolCategory.PHILOSOPHY or category == ToolCategory.OVERKILL:
            self.accountant.charge(HeatTaxLevel.L2_MEANING, estimated_tokens * 0.001,
                                   f"Tool: {tool_name} ({category.value})")
        elif category in (ToolCategory.REPEAT, ToolCategory.CACHE):
            self.accountant.charge(HeatTaxLevel.L1_LOGICAL, estimated_tokens * 0.001,
                                   f"Tool: {tool_name} ({category.value})")
        else:
            self.accountant.charge(HeatTaxLevel.L0_PHYSICAL, estimated_tokens * 0.001,
                                   f"Tool: {tool_name} ({category.value})")
...
if __name__ == "__main__":
    ...
    from mssclaw.core.tool_budget_gate import ToolBudgetGate, ToolCategory
    # NOTE: heat_tax_accountant.py was never implemented;
    # using HeatTaxBudget from heat_tax.py as nearest substitute

    acc = HeatTaxBudget(max_tokens_per_turn=1000, l2_ratio_warning=0.3)
    ...
    acc.charge(HeatTaxLevel.L2_MEANING, 0.4, "simulate high L2")  # Simulate high L2 usage
