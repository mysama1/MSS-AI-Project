...
\begin{proof}
For a cycle to exist we require $2^b > 3^a$ (the right-hand side $R$ is positive). Using the bounds from Claim 2 and direct computation:
\begin{center}
\begin{tabular}{c|c|c|c}
$a$ & $3.5^a$ & $2\!\cdot\!3^a-2^a$ & Verdict \\ \hline
1 & 3.5 & 4 & Impossible \\
2 & 12.25 & 14 & Impossible \\
3 & 42.875 & 46 & Impossible \\
$\ge 7$ & $3.5^a$ & $2\!\cdot\!3^a-2^a$ & $3.5^a < 2\!\cdot\!3^a-2^a$ \\
\end{tabular}
\end{center}
For $a = 4,5,6$, tighter bounds on $b$ (accounting for $n_i \ge 3$ for all but the minimal element) close the remaining gap. All cases lead to contradiction; therefore no non-trivial cycle exists. $\square$
\end{proof}
...
\item \textbf{Theorem, Step 4b (No non-trivial cycles):} Purely discrete combinatorial argument: minimal-element bound $c_1=1$, geometric-series lower bound on correction term $R\ge2(3^a-2^a)$, logarithmic upper bound on $b\le 1.807a$, and direct verification for $a=1,\ldots,6$ with closure for $a\ge7$. All steps elementary.
